...
import com.zcq.hostmodeble.ble.utils.BleLruHashMap;
...
/**
 * Create by zcq on 7/8/22 11:56 AM
 */
public class MultipleBluetoothController {
    ...
    public MultipleBluetoothController() {
        bleLruHashMap = new BleLruHashMap<>(BleHelper.getInstance().getMaxConnectCount());
    ...
    public void refreshConnectedDevice() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            ...
            for (int i = 0; bluetoothList != null && i < bluetoothList.size(); i++) {
                ...
                if (!BleHelper.getInstance().isConnected(bleBluetooth.getDevice())) {
